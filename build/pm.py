...
def annual_reported_band(staging: dict) -> dict:
    ...
    return delivery_band(
        ...
        extra_note=(
            ...
            "<b>更要紧的是这条指引一直带着一句排除条款</b>：自 2008 年分拆到 2022 年 2 月的"
            "那一期为止，每一次全年预测都写明它不含未来并购、未预料到的资产减值与退出成本、"
            "以及任何异常事件。所以这个挂着 GAAP 名字的数并不是对 GAAP 的预测，"
...
def build_payload(staging: dict) -> dict:
    ...
    return {
        ...
        "notes": [
            ...
            "自 2008 年分拆到 2022 年 2 月那一期为止，每一次全年预测都附同一句排除条款：不含未来并购、"
            "未预料到的资产减值与退出成本、以及任何异常事件。2022 年第二季起公司改用「报告口径 + 逐项"
