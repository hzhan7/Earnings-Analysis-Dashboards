...
def build_payload(staging: dict) -> dict:
    ...
    return {
        ...
        "notes": [
            ...
            "2016Q1–2022Q4 里有五个季度（2019Q4、2020Q2、2020Q3、2020Q4、2022Q4）的数字在当期发布与一年后的对照列之间不一致，其中只有 2022 年第四季动到了收入：营业利润与净利润各下调 211、收入下调 27（₩十亿）；另外四次只动利润两行，幅度 0.1%–3%。五次都不带「重述」字样，都只出现在四个季度后的「去年同期」列上，而每期发布都声明自己在外部审计完成之前编制。本页 42 季序列采用「当期首次发布」那一版，因为只有它能让 2022 年四季加总回到公司当时印出的全年数。2022 年前三季此前按英文发布正文的精度记（Q1、Q2 只到百亿），而 Q3 的营业利润与净利润记的是 2023 年第三季对照列里那一版、不是它自己那期印的 —— 2026-08-31 全部改回当期首次发布，并改用韩文发布表的 억원 精度。该年四季加总与全年数的残差因此从约十亿降到 ±0.6，本页对 2022 年的求和容差也从 15 收到 1。",
