...
def routine_charts(staging: dict) -> list[dict]:
    ...
    # The diluted share count has to come off that same axis, and not because it
    # is also untouched: the note below divides the fall in share count against
    # the ratio of the two multiples, so taking the multiples over 42 quarters
    # while taking the share count over the recast window's 38 compares two
    # different spans inside one sentence. That is how this note came to print
    # "from 903M" -- 903M is 2017Q1, the recast window's first quarter -- while
    # the multiples beside it already started at 2016Q1, and how a 7.7pp span
    # mismatch got read out as preferred dividends.
    long_labels = staging["period_labels"]
