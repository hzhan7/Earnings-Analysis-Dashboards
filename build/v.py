...
def track_incentive_rate(staging: dict, group: list[dict], mode: str, key: str) -> dict:
    lines = staging["revenue_lines_usd_m"]
    ...
    return threshold_lines_exhibit(
        headline_for("激励率", f"{full_rate[-1]:.2f}%", group, mode, key),
        long_labels, [{"name": "实际激励率", "values": rounded(window), "color": "NAVY"}], group, mode,
...
def track_intl_yoy(staging: dict, group: list[dict], mode: str, key: str) -> dict:
    ...
    return threshold_lines_exhibit(
        ...
        long_labels, [{"name": "International transaction YoY", "values": rounded(yoy), "color": "NAVY"}],
...
TRACKS = {
    "revenue_lines_usd_m.incentive_rate_pct": track_incentive_rate,
...
def build_payload(staging: dict) -> dict:
    ...
    excluded = list(next_kpi.get("excluded", [])) if next_kpi else []
