...
def build_payload(staging: dict) -> dict:
    ...
    return {
        ...
        "notes": [
            ...
            "backlog 相关的三个数各有各的口径：总额与不可撤销 FSA 承诺是公司披露值（精度 US$0.1B），「未来 12 个月内可确认的比例」在申报原文里是对<b>扣除 FSA 之后</b>的 backlog 说的，因此本页的可确认额 =（总额 − FSA）× 该比例，为自算值。",
            "本页<b>不发布</b>剔除 Ansys 之后的季度 EDA 收入或其增速。公司从未在任何申报文件中单列 Ansys 的实际季度收入，只在 FY2026 收入指引的脚注里给出预期口径，因此该拆分在公开申报文件里无法复算；能复算的那一部分（年度指引层面的并购与原生拆分）已单独成图。",
