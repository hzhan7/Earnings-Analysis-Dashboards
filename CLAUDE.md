...
**开工第一件事**：`git rebase main`（**本地** main）。你的 worktree 建出来的那一刻起
main 就在动，别把建 worktree 时的基线当固定值。2026-09-19 起所有者手动 push，本地 main
常常领先 origin/main（已合并、还没推），所以基线是本地 main，不是 origin/main。

## 0. 工作约定（2026-09-19 所有者定，优先于下文任何一节）

- **做完默认合并到本地 main**，不问要不要 commit / push。**不推远端**：等所有 worktree 都进了
  本地 main，所有者自己审、自己手动 push（他明说「push up」时才推）。
- 不开 ultracode、不起 Workflow。同一家公司的页同一时间只让一个会话改
  （下文 §3 的协调者流程因此很少再需要）。
- 指图用**标题**，不用图号。
- 页面上的数一律用**官方公布值**；自算值与官方值不一致时用官方值，把差异写进给所有者的总结。
- **卖方一致预期可以放**（2026-09-24 所有者定）：标注「市场预期」、注明取数日期，不点名券商或数据商
  （与 README 内容边界一致）。放不放是各页按报告与来源决定的事，没有可核对的带日期来源就不放。
- **电话会才说过的数和话能上页、能拿来结算**（2026-09-24 所有者定），但每一处都要标明是电话会，
  不混作申报值。
- 图表是模版 + 数据；刻度、字体这类不影响阅读的小问题不花大力气。
- 生成物冲突：`data/*.js` 与 `*/index.html` 在 `.gitattributes` 里是 `merge=binary`，冲突时不插标记、
  保留本分支那份但停在 unmerged —— 不要手挑，跑 `python3 build/all.py` 再 `git add` 它们（§4）。
...
- **排程和授权是两件事，不要混。** 多个会话并行时会出现一个协调者，它可以决定
  ...
  推到共享远端是对外动作。2026-09-19 起的常设规矩是：**谁都不推，做完合并到本地 main，
  由所有者人工 push**（见 §0）；同伴会话或协调者的 OK 改变不了这一条。
...
不要手工合并它们（2026-09-19 起 `.gitattributes` 把它们设成 `merge=binary`：冲突时文件里不再有冲突标记，
工作区是本分支那份、状态是 UU —— 下面这段照样适用）：
...
- **对照两个读数之前，先确认它们读的不是同一份东西。** 上面那条讲的是怎么**防住**，
  ...
  **一个对每一对都成立的统计量，对「哪一对」按构造无分辨力。** 2026-09-22 查「钟表的弱是
  品类周期还是某一家的事」时，两条钟表线的相关系数是 **+0.93**，看起来像铁证。可同一窗口上
  四条线两两相关一共 6 对，**最弱的一对也有 +0.76** —— 共同周期把一切都拉到一起，所以
  「这两条相关很高」对「它们共享的是不是钟表这个品类」什么也没说。能分辨的是**水平**：
  两家的钟表线都低于自家主力线十几个百分点，而两家的主力线是完全不同的东西。
  判据很朴素：**把同样的统计量算给几对无关的线看看**，它要是对谁都成立，就不能用来指认谁。
  这和下面那条「语料相同」是同一族 —— 那条是两个读数共享输入，这条是一个读数对所有候选
  都给同一个答案；两者都长得像证据。
...
**而闸门要活下来，靠的是它拒绝报警的地方。** 越界检查只查纵轴：横向不查，因为
end_label 与柱标本来就故意画在边距里。描边（`fill="none"`）的 path/polyline 以 viewBox
为界；填充形状（柱、堆叠块、菱形）以渲染器在 `<svg data-plot="上沿 下沿">` 里声明的
绘图区为界、容差 1px —— 基线与堆叠块本来就贴着绘图区边沿，容差就是为它们留的；菱形按
中心判，中心才是数据点。**但豁免要划到它要放过的那一类为止。** 原先写的是「`fill` 不为
none 的不查，因为堆叠面积块本来就贴边」—— 为堆叠块写的豁免划在了一个属性上，于是全站
每一根柱子一起被放过：2026-09-17 接 Kering 时，全负的 `grouped_bars` 让轴顶落在 0 之下、
柱从画布外起笔，这道闸门按构造看不见。补上时还量出一层：以 viewBox 为界也不够，全负柱
里最小的柱相对最大的越小，越界越少，snps Ex25 改成 −50..−1,211 时柱只越过绘图区顶 12px、
仍在 viewBox 之内。同理，NaN 要用 token 边界
...
迁移顺手查出的手写错，比迁移本身的改动更值钱（第一批 MU 8 处、NKE 9 处、AVGO 13 处、COST 5 条，此后每页都有）。
按类记在下面，每条括号里是一个例子页。

**哪里藏着手写错**

- 窗口拉长、加节、回补之后没跟着改的手写句是最大的一类（「首次给这条指引」在第二次之后仍写着，全窗口最大值被安到
  另一件事上）。中文数字（「十八季」）、轴标签（「Q1'17」）、位置词（「2024 年上半年」）census 测试看不见，只能逐句读
  （MA；AVGO）。
- 全称词（单向 / 一直 / 全部 / 唯一 / 第一次 / 每一季 / 本站唯一）逐个核，反例常在本页另一张图里；「本站唯一」
  会随站点变大而失效（AMZN）。
- 标题与它自己的图注矛盾、board 生成的半句与手写的半句矛盾，是最便宜的检查（SPGI）。
- record 要看记的是什么：新闻稿说创纪录的是交易量，不是收入（SCHW）。
- 同一件事的多份手写副本可能都错；两段说法互相矛盾时回一手申报核，别挑一段信（PM 三份都错；COST FY1995）。
  「例子」要对着它所说的方法核（RMS）。

**数从哪取、跟谁比**

- 阈值块手敲的当前值会算错、会翻转结论，取整会翻末位；当前值一律从序列读（SCHW 费用阈值由安全变越线）。
- 官方值与自算值在印出精度上比；差值用官方两端相减，由它推出的数一起换（MC）。两者常常不是同一个量：MSFT 回购
  含不含代扣税、META 地域口径、AMZN 汇率口径、HKEX EBITDA 分母、V 回报的交易日口径。
- 去年同期用本季新闻稿重印的值：没有「重述」标签时首印与重印也会不同（NDAQ 毛额）。「精确相等」的勾稽可能正是
  手录错误的症状（META）。
- 取值方式会错：
  - `min()` 取到的不是回撤谷底（SPGI）；
  - 窗口往前补之后，固定下标和 `[0]` 会错位；句子里配对的两个数要从同一个窗口位置取（CBOE）；
  - 按这张图自己的序列算，不拿几条的合计（SK hynix）；
  - 范围要对句子说的全部对象算（MCO）；
  - 普查范围不等于窗口长度，普查数放进 series 并用测试绑定（SK hynix）。

**披露与来源**

- 「不披露」「没有任何文件」要查 10-Q 正文、别的表和电话会（V 支付额、MA 返点金额、AMZN 分国家收入、SK hynix
  电话会口头）。「只申报了年初至今」对第一季永远不成立；XBRL 标签跨年换名会造出假空格；管道边界不等于披露边界（AMZN）。
- 公司的政策目标要在本季申报里核，8-K 与 8-K/A 要分清；「公司明确说」要核对是哪份文件说的（SPGI 85%）。
- sources 要含本季自己那份发布；链接直链到文件、能打开，测试抓不到 404。EDGAR browse 短链会返回 503，老申报取完整
  的 .txt；IR 站返回 403 时从 SEC 取同一份 EX-99.1（PM）。
- 本地笔记的文件名可能标错季度，按内容对季度；第四季新闻稿里开的是下一年的指引，取「本期新闻稿更新的那条记录」（PM）。

**格式**

- 图注里不原样拼 markdown 星号（AMZN）；零值会被格式化成 −0.0% 或 -0.00pp（MA）；「涨了 N 倍」不等于「涨到 N 倍」
  （NDAQ）；动词已带方向时印绝对值（CBOE「掉了 -11.9」）；倒推额度要说清按区间的哪一端（CBOE）。

**series 的写法**

- 同一个带期号戳的块只读一次，再往下传（CBOE）。
- 键名不嵌年份；能从序列推出的事实不另存一份，包括由季数换算出的「两年」（NKE `fy2026_*`；Samsung）。
- series 文件的缩进各不相同（2 或 1，有的结尾带换行）：改完按原缩进、原结尾写回，不然 diff 满屏，逐字节比对也看不出
  改了什么。

**演练**

- 回退和前滚都要做，逐句读结果；季度形态换着来：会计 Q4、半年报的奇数季、只有速报的季（COST「同店两个精度」那句
  下一季就会抛异常）。
- else 分支的句子、结算年的口径错配，只有演练才抓得到（SPGI 分拆）。
- git 历史里没有上一季原件时，回退演练只能证明页面能构建、句子通顺（Samsung）。

**测试**

- 计数钉按 series 现算，图号从页面读（KER）；期望值在测试里独立算，不调用 builder 自己的函数（HKEX）。
- 做变异测试前先 `find build tests -name __pycache__ -prune -exec rm -rf {} +`，否则「变异没打中」看起来和「测试没反应」
  一模一样；变异里「逼假」的状态只打破被测的那一个条件（MA）。
- 在 `git archive` 抽取出的目录里跑测试，要先 `git init`：content_boundary 用 `git ls-files`，不然那几个 ERROR 是环境
  造成的假红（KER）。
- 多个 agent 并行时，临时脚本放在各自的子目录；「全绿且失败列表为空」要当成可疑结果，先确认被瞄准的那条真的跑了
  （并行的各批迁移）。
- **窗口拉长之后，守全称命题的那套测试会从两头失效，而两头都表现为绿。** 回补 LVMH 半年利润（7 期 → 21 期）时
  同时中了两种（MC）：
  - 反例测试的**正对照**没了。`assertIn(措辞, 真实 payload)` 在窗口是三年时成立，十年之后那句话本该消失、也确实
    消失了 —— 于是同一条测试的 `assertNotIn` 开始对一句**任何分支都产生不了**的话断言，永远为真。修法不是删掉
    这条 case，是在测试里**造出让它成立的那棵树**（把每个下半年压到比上半年薄），在那棵树上断言措辞出现，再变异
    它断言措辞消失。
  - **一个条件在真实数据上本来就为真，把它改成无条件就打不中。** 新写的「两条链的转折年份相同才这么说」，变异成
    `if True` 之后全绿：`assertEqual(same, 措辞 in text)` 两边都是 True。要补的是一个 `same` 为假的反例 ——
    这里是「某年下半年只薄 0.001pp，小于页面的印出精度」，两条链的长度就分开了。
  判据同 §5：**一个你还没挣到的绿灯**。变异台架这两种都报绿，只有回头问「这条断言在什么情况下会红」才认得出。
...
### 10.1 回补一条长序列（2026-09-22 做爱马仕 2016–2024 时记的）

**每一格读两次，而且是从两份不同的文件读。** 爱马仕每份发布都重印上年同期列，所以
2017Q1–2024Q2 的每个季度都有两个来源：它自己那份发布的本期列，和次年那份发布的上年同期列。
四个抄录者按年分工，谁也没读过另一个人的结果，最后逐格比对：**跨文件 0 处不一致**，
38 个可比季度上「上年同期列 == 四季前的本期列」全部成立 —— 这同时也证明了公司没有重述过。
这比「同一份文件读两遍」强，因为两次读的是不同的纸。

**「不一致」里绝大多数是精度切换，别把它当冲突。** 爱马仕 2017–2021 印到 0.1 百万欧元，
2021 年全年发布起改印整数。两份文件对同一格给出 1351.7 与 1352 不是矛盾。判据要写成
「一边是整数且四舍五入后相等」，而且**四舍五入要按半进位**：Python 的 `round(434.5)` 是 434
（银行家舍入），公司印的是 435，于是三格被误报成冲突。反过来也有：382.5 被公司印成 382。
所以真正的判据是「|差| ≤ 0.5 且一边是整数」。

**公司自己印的分项加不回公司自己印的合计。** 整数年代最大差 €2M（2023Q3 板块），
小数年代最大差 €0.1M。原有测试的容差是 ±1，写着「每行都印到百万，所以 ±1 就是全部容差」——
那句话在八季窗口上成立，在 42 季上不成立。**重新量，把量出来的上界钉住，并说明是哪一季逼出来的**，
不要顺手放宽。四个季度相加与公司申报的全年也一样：2023 年差 €1M（公司印的上半年 6,698
比第一、二季相加多 1），其余九年为零。原来的断言是 `assertEqual`，改成「差多少就说多少」——
**只在通过时才开口的检查不是检查**。

**窗口拉长之后，条件句会自己关掉，那是对的。** 两句话在 42 季窗口上不再成立并自动消失：
「有时候是去年太差」需要某条加速线的上季读数正好是**窗口低点**，而 2020 年的坑比之后任何一格都低；
「符号相反的格子全部落在日本与亚太」在 38 个有增速的季度里变成 15 格、横跨三个地区。
测试原本断言这两句**必然出现**，于是变红。正确的修法不是删句子，是把断言改成双向的：
造一组让条件成立的数据，断言句子**回来**（本次给美洲那三格翻正，「全部落在」立刻重现）。

**回补带来的空格会一路传到最上层。** 2016 年四个季度只有欧元金额、没有增速（公司出版物页的
年份标签从 2017 开始，而 2016 年的增速要用 2015 年的季度算）。于是 builder 里每一个
`min()` / `max()` / `sum()` / `index()` 跑在增速数组上的地方都要先过滤 —— 一共 12 处，
其中 6 处在测试里。做法是加一个 `rated_quarters()`：**只有同时印出两种增速的季度才进普查**，
并且**测试自己再写一遍这个函数而不是 import 它** —— 借用 builder 对「哪些季度算数」的判断去
检查 builder，按构造检查不出它算错了范围。连带地，所有「N 个季度里有 M 个……」的句子分母
都要从 42 改成 38，否则页面会用一个没有增速的季度当分母。
