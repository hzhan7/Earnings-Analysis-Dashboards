...
- **迁移顺手查出的手写错，比迁移本身的改动更值钱**：MU 8 处、NKE 9 处、AVGO 13 处、COST 5 条。形态几乎都是
...
- **回退演练不是走过场**：COST 第一次演练就停在「同店销售两个精度」那句 —— 它取最后三季，而会计 Q4 没有
  10-Q、没有整数版，下一季（FY2026 Q4）正常换季就会在这里抛异常。当季数据形态（会计 Q4、半年报季、只有速报
  的季）换一下就炸的句子，只有退一季、进一季才看得见。
- **两段手写说明互相矛盾时，回一手申报核，别挑一段信**：COST 的图注写 FY1995 计划 US$550–700M，注释引的原文是
  「$600 million to $700 million」；FY1994 10-K 原文是后者，series 里存的 550 才是错的。
