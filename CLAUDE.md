...
| `exchanges` | 交易所 | 10 | ← **已预分配，尚未落地**：CME 与 CBOE 两个会话同时需要它，各自本地已写好一字不差的同一行、同样追加在末尾。第三个交易所页直接用这个键，不要另造。|
...
- **注入必须自己带断言。** `str.replace` / `sed` / 正则替换在找不到目标时**静默
  ...
  三个人各中一次：两次靠 `assert old in s`（或注入后 diff 非空）拦住，一次没拦住
  —— 那次拿同一份文本跑了两个正则，得出「两边都正常」，差点回报「复现不出来」。
