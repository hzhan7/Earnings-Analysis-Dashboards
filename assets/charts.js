...
(function () {
  ...
  /* 竖排长标签的**实测**收缩：把已经画出来的文字缩到不超过 maxLen（user 单位）。
     为什么不能像别处那样按字数估：这里的文案是 payload 给的中英混排
     （enx 那条「债券含 Euronext ABM、新增挂牌口径扩至所有类型（均已重述）」），
     汉字约 1 em、拉丁字母约 0.55 em，按字数乘系数必错一大截。
     getComputedTextLength() 是浏览器算好的真实长度，直接拿来用。
  ...
  /* 一段文字有多宽，以 em 计（乘上字号就是 px）。汉字与全角标点按 1 em，其余按 0.55 em。
     ...
     这是估算不是实测（实测要进渲染树，而这里在建 SVG 之前就要用），
     只用来定**上界**，估宽一点只会让字保守一点，不会造成越界。 */
  function emWidth(s) {
    var w = 0, k, ch;
    s = String(s == null ? '' : s);
  ...
  function draw(host, ex, opt) {
    ...
    /* x 标签的字号上界：多数图的 x 是月份（3~6 字，怎么放都够），但有的图 x 是**名字**
       ...
       躲过去了，一放大就实打实压 94px。所以按最长标签反推上界：
         rot=0   横排：整条要塞进一格           → band / (len×0.55)
    ...
    /* 两条约束取严的那条：
       ...
       （「新台币汇率 vs 假设（升值为逆风）」20+ em）它是唯一挡得住的约束 ——
       那种图上标签本来就顶着标签带的下沿，字号再放大就直接盖到正文上去了。 */
